...
def create_data_structure(string_input):
    network = {}
    tmp_input = string_input[:]
    if string_input == '':
        return network
    while tmp_input:
        list_connections =[]
        list_games = []
        name_i = tmp_input.find('is')
        name = tmp_input[0:name_i - 1]    
        connections_i = tmp_input.find('to') 
        connections_f = tmp_input.find('.')
        connections = tmp_input[connections_i + 3:connections_f] 
        list_connections = connections.split(', ')
        tmp_input = tmp_input[connections_f + 1:]
        games_i = tmp_input.find('play')
        games_f = tmp_input.find('.')
        games = tmp_input[games_i + 5: games_f] 
        list_games = games.split(',')
        tmp_input = tmp_input[games_f + 1:]
        network[name] = {'connections' : list_connections, 'games' : list_games} 
    return network
...
def add_connection(network, user_A, user_B):
    if user_A not in network or user_B not in network:
        ...
        if user_A not in network[user_B]['connections']:
            network[user_B]['connections'].append(user_A)
    return network
...
def get_secondary_connections(network, user):
    ...
    if user not in network:
        return None 
    if network[user]['connections'] == None:
        return []
    else:
        list_of_connections = network[user]['connections']
...
def connections_in_common(network, user_A, user_B):
    if user_A or user_B not in network:
        return False
    num = 0
    list_of_connections_A = network[user_A]['connections']
    list_of_connections_B = network[user_B]['connections']  
    for connectionA in list_of_connections_A:
        for connectionB in list_of_connections_B:
            #print connectionA, connectionB
...
def path_to_friend(network, user_A, user_B): 
	...
	['John', 'Bryant', 'Olive', 'Ollie', 'Mercedes', 'Walter', 'Levi']
	"""
	visited = {}
	parents = {}
	path = []
	...
	if user_A not in network or user_B not in network:
		return None
	user = user_A
	def DFS(network, parents, user, user_B):
		visited[user] = True
		if user_B not in network[user]['connections']:
			for connection in network[user]['connections']:
				if not visited[connection]:
					parents[connection] = user
					return DFS(network, parents, connection, user_B) 
		else:
			parents[user_B] = user
			return parents
		return parents
	DFS(network, parents, user, user_B)
	if user_B in parents:
		parent = parents[user_B]
		path.append(user_B)
		while parent != user_A:
			path.append(parent)
